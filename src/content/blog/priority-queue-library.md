...
description: "Pythonの標準ライブラリheapqが使いづらすぎたので、PyPy環境でheapqよりも高速に動くライブラリを自作しました。"
pubDate: 2026-04-01
tags: ["Python", "アルゴリズム", "競技プログラミング"]
---
...
理由は単純で、CPythonの`heapq`はC言語で実装されていて非常に高速に動作するからです。CPythonではユーザーのコードがそれと同等に高速に動作することはまずありません。
...
圧勝ですね。タプル操作の重さが`heapq`の実行時間に表れています。それに比べて自作の方はタプル比較をしていないのでパフォーマンスの悪化が抑制されています。$N=2\times 10^5$ で $2\sim 3$ 倍、$N=2\times 10^6$ で 約 $3$ 倍の差が出ています。
...
見事に高速化できています。最適化前の`priorty_queue`と比較しても、約 $2\sim 3$ 倍の速さを実現できていますし、要素が増えても全く遅くなっていません。
